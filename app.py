...
ACTIVE_SKILL = "datenschutz"  # Multi-Skill: aktiver Default-Skill

# ── Multi-Skill: Manifest & Prompt Loader ───────────────────────────────────
import json as _json_ms

def load_skills_manifest() -> dict:
    """Laedt skills.json aus dem Projektverzeichnis."""
    _p = os.path.join(os.path.dirname(os.path.abspath(__file__)), "skills.json")
    try:
        with open(_p, encoding="utf-8") as _f:
            return _json_ms.load(_f)
    except Exception:
        return {"active_skill": "datenschutz", "skills": {}}


def load_system_prompt(skill_id: str = "datenschutz") -> str:
    """Laedt den System-Prompt fuer einen Skill aus prompts/{skill_id}.md."""
    _base = os.path.dirname(os.path.abspath(__file__))
    _path = os.path.join(_base, "prompts", f"{skill_id}.md")
    try:
...
def get_collection_name(skill_id: str = "datenschutz") -> str:
    """Gibt den ChromaDB-Collection-Namen fuer einen Skill zurueck."""
    manifest = load_skills_manifest()
    return manifest.get("skills", {}).get(skill_id, {}).get(
...
SYSTEM_PROMPT = load_system_prompt(ACTIVE_SKILL)
...
PWA_HEAD = (
    ...
    '    text+="Antwort: "+answer;'
    '    /* Force plain text copy via hidden textarea (iOS compatible) */'
