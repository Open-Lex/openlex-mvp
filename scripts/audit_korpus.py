...
def main():
    ...
    # Suspected confusion pairs — exakte Treffer (kein Substring-Match um false positives zu vermeiden)
    CONFUSION_PAIRS = [
        ("TTDSG", "TDDDG"),   # Alte vs. neue Bezeichnung
        ("BDSG-neu", "BDSG"),
        ("BDSG2018", "BDSG"),
        ("DS-GVO", "DSGVO"),   # Alternative Schreibweise (beide ok, aber inkonsistent)
    ]
    DEPRECATED = {"TTDSG", "TMG", "TKG"}  # Veraltete Gesetze die durch TDDDG ersetzt wurden
    found_issues = []
    for a, b in CONFUSION_PAIRS:
        has_a = a in known_gesetz_keys
        has_b = b in known_gesetz_keys
        if has_a and a in DEPRECATED:
            found_issues.append(f"⚠️  Veraltete Bezeichnung `{a}` vorhanden (sollte `{b}` sein?)")
        elif has_a and has_b:
            found_issues.append(f"⚠️  Beide `{a}` und `{b}` vorhanden — inkonsistente Schreibweise")
    ...
    # Artikel-Verteilung (Werte enthalten bereits "Art. X" — nicht nochmal "Art." vorne)
    ...
    # Korrekter Vergleich: Wert ist "Art. 4", nicht "4"
    art4_chunks = [c for c in dsgvo_chunks
                   if c["meta"].get("artikel", "") in ("Art. 4", "4")]
